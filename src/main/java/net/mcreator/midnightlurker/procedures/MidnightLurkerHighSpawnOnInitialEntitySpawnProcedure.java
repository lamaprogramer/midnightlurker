...
public class MidnightLurkerHighSpawnOnInitialEntitySpawnProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		lurker = new File((FMLPaths.GAMEDIR.get().toString() + "/config/"), File.separator + "lurkerconfig.json");
		{
			try {
				...
				if (mainjsonobject.get("lurker_low_spawn_rate").getAsBoolean() == false) {
				...
				} else if (mainjsonobject.get("lurker_low_spawn_rate").getAsBoolean() == true) {
					MidnightlurkerMod.queueServerWork(6, () -> {
						if (!entity.level.isClientSide())
							entity.discard();
					});

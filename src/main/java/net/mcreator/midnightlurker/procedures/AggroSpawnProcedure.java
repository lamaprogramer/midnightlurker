	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("InsanityStage") == 7) {
			if (!(!world.getEntitiesOfClass(MidnightLurkerAggressiveEntity.class, AABB.ofSize(new Vec3(x, y, z), 700, 700, 700), e -> true).isEmpty())) {
				spawnx = x + Mth.nextInt(RandomSource.create(), 2, 12);
				spawny = y + Mth.nextInt(RandomSource.create(), 0, 12);
				spawnz = z + Mth.nextInt(RandomSource.create(), 2, 12);
			}
			if (world.isEmptyBlock(new BlockPos(spawnx, spawny + 0, spawnz)) && world.isEmptyBlock(new BlockPos(spawnx, spawny + 2, spawnz)) && world.isEmptyBlock(new BlockPos(spawnx, spawny + 3, spawnz))
					&& !world.isEmptyBlock(new BlockPos(spawnx, spawny - 1, spawnz))) {
					...
					MidnightlurkerMod.queueServerWork(1, () -> {
						if (world instanceof Level _level) {
							if (!_level.isClientSide()) {
								_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("midnightlurker:lurkerdistantscream")), SoundSource.NEUTRAL, 1, 1);
							} else {

...
//        });
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            if (mapdata != null)
                ServerPlayNetworking.send(handler.getPlayer(), MidnightlurkerMod.CHANNEL_ID_VARIABLES, PacketByteBufs.create().writeVarInt(0).writeNbt(mapdata.writeNbt(new NbtCompound())));
            if (worlddata != null)
                ServerPlayNetworking.send(handler.getPlayer(), MidnightlurkerMod.CHANNEL_ID_VARIABLES, PacketByteBufs.create().writeVarInt(1).writeNbt(worlddata.writeNbt(new NbtCompound())));

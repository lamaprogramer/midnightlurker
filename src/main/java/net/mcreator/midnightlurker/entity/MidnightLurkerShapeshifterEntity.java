...
public class MidnightLurkerShapeshifterEntity extends PathfinderMob implements IAnimatable {
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("shapeshifterwalk", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("shapeshifteridle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
